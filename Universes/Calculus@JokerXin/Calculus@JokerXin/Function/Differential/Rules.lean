...
theorem Deriv.SMul {F : Function} {a x₀ L : ℝ}
    (h_f : Deriv F x₀ L)
  : Deriv (Constant a * F) x₀ (a * L)
:= sorry

/-- ### 导数数乘（表达式）
    ### Derivative Scalar Multiplication (Expression) -/
theorem DerivExpr.SMul {f : ℝ → ℝ} {a x₀ : ℝ}
  : D (a • f) x₀ =? the a * D f x₀
:= sorry

/-- ### 左导数数乘
    ### Left Derivative Scalar Multiplication -/
theorem LeftDeriv.SMul {F : Function} {a x₀ L : ℝ}
    (h_f : LeftDeriv F x₀ L)
  : LeftDeriv (Constant a * F) x₀ (a * L)
:= sorry

/-- ### 左导数数乘（表达式）
    ### Left Derivative Scalar Multiplication (Expression) -/
theorem LeftDerivExpr.SMul {f : ℝ → ℝ} {a x₀ : ℝ}
  : D₋ (a • f) x₀ =? the a * D₋ f x₀
:= sorry

/-- ### 右导数数乘
    ### Right Derivative Scalar Multiplication -/
theorem RightDeriv.SMul {F : Function} {a x₀ L : ℝ}
    (h_f : RightDeriv F x₀ L)
  : RightDeriv (Constant a * F) x₀ (a * L)
:= sorry

/-- ### 右导数数乘（表达式）
    ### Right Derivative Scalar Multiplication (Expression) -/
theorem RightDerivExpr.SMul {f : ℝ → ℝ} {a x₀ : ℝ}
  : D₊ (a • f) x₀ =? the a * D₊ f x₀
:= sorry

/-- ### n阶导数数乘
    ### N-th Order Derivative Scalar Multiplication -/
theorem NthDeriv.SMul {n : ℕ} {F : Function} {a x₀ L : ℝ}
    (h_f : NthDeriv n F x₀ L)
  : NthDeriv n (Constant a * F) x₀ (a * L)
:= sorry

/-- ### n阶导数数乘（表达式）
    ### N-th Order Derivative Scalar Multiplication (Expression) -/
theorem NthDerivExpr.SMul {n : ℕ} {f : ℝ → ℝ} {a x₀ : ℝ}
  : Dₙ n (a • f) x₀ =? the a * Dₙ n f x₀
:= sorry
  ...
  | some A =>
    ...
    | some B =>
      ...
      have h_fg_eq : D (f + g) x₀ = the (A + B) := by
        ...
        simp only [HAdd.hAdd, Add.add, Function_op₂]
        rw [Set.inter_self]
      rw [h_fg_eq]
      rfl
...
theorem NthDeriv.Add {n : ℕ} {F G : Function} {x₀ A B : ℝ}
    (h_f : NthDeriv n F x₀ A) (h_g : NthDeriv n G x₀ B)
  : NthDeriv n (F + G) x₀ (A + B)
:= sorry
  ...
  | some A =>
    ...
    | some B =>
      ...
      have h_fg_eq : D (f - g) x₀ = the (A - B) := by
        ...
        simp only [HSub.hSub, Sub.sub, Function_op₂]
        rw [Set.inter_self]
      rw [h_fg_eq]
      rfl
...
theorem NthDeriv.Sub {n : ℕ} {F G : Function} {x₀ A B : ℝ}
    (h_f : NthDeriv n F x₀ A) (h_g : NthDeriv n G x₀ B)
  : NthDeriv n (F - G) x₀ (A - B)
:= sorry
...
theorem Deriv.Mul {F G : Function} {x₀ A B : ℝ}
    (h_f : Deriv F x₀ A) (h_g : Deriv G x₀ B)
  : Deriv (F * G) x₀ (A * G.map x₀ + F.map x₀ * B)
:= sorry
  ...
  | some A =>
    ...
    | some B =>
      ...
      have h_fg_eq : D (f * g) x₀ = the (A * g x₀ + f x₀ * B) := by
        ...
        simp only [HMul.hMul, Mul.mul, Function_op₂]
        rw [Set.inter_self]
      rw [h_fg_eq]
      rfl
...
theorem LeftDeriv.Mul {F G : Function} {x₀ A B : ℝ}
    (h_f : LeftDeriv F x₀ A) (h_g : LeftDeriv G x₀ B)
  : LeftDeriv (F * G) x₀ (A * G.map x₀ + F.map x₀ * B)
:= sorry
...
theorem RightDeriv.Mul {F G : Function} {x₀ A B : ℝ}
    (h_f : RightDeriv F x₀ A) (h_g : RightDeriv G x₀ B)
  : RightDeriv (F * G) x₀ (A * G.map x₀ + F.map x₀ * B)
:= sorry
...
theorem Deriv.Div {F G : Function} {x₀ A B : ℝ}
    (h_f : Deriv F x₀ A) (h_g : Deriv G x₀ B)
    (h_g_ne_0 : G.map x₀ ≠ 0)
  : Deriv (F / G) x₀ ((A * G.map x₀ - F.map x₀ * B) / (G.map x₀)^2)
:= sorry
...
:= by
  ...
  | some A =>
    ...
    | some B =>
      ...
      have h_g : Deriv ⟨g, Iii⟩ x₀ B :=
        DerivExpr_to_Deriv h_B
      have h_fg_eq : D (f / g) x₀ = the ((A * g x₀ - f x₀ * B) / (g x₀)^2) := by
        apply Deriv_to_DerivExpr
        convert Deriv.Div h_f h_g h_g_ne_0 using 2
        simp only [HDiv.hDiv, Div.div, Function_op₂]
        rw [Set.inter_self]
      rw [h_fg_eq]
      have h_denom_neq : the ((g x₀)^2) ≠ the 0 := by
        intro h
        injection h with h_eq
        apply h_g_ne_0
        exact sq_eq_zero_iff.mp h_eq
      simp only [HDiv.hDiv, Div.div, h_denom_neq]
      rfl
...
theorem LeftDeriv.Div {F G : Function} {x₀ A B : ℝ}
    (h_f : LeftDeriv F x₀ A) (h_g : LeftDeriv G x₀ B)
    (h_g_ne_0 : G.map x₀ ≠ 0)
  : LeftDeriv (F / G) x₀ ((A * G.map x₀ - F.map x₀ * B) / (G.map x₀)^2)
:= sorry
...
:= by
  ...
  | some A =>
    ...
    | some B =>
      ...
      have h_fg_eq : D₋ (f / g) x₀ = the ((A * g x₀ - f x₀ * B) / (g x₀)^2) := by
        apply LeftDeriv_to_LeftDerivExpr
        convert LeftDeriv.Div h_f h_g h_g_ne_0 using 2
        simp only [HDiv.hDiv, Div.div, Function_op₂]
        rw [Set.inter_self]
      rw [h_fg_eq]
      have h_denom_neq : the ((g x₀)^2) ≠ the 0 := by
...
theorem RightDeriv.Div {F G : Function} {x₀ A B : ℝ}
    (h_f : RightDeriv F x₀ A) (h_g : RightDeriv G x₀ B) (h_g_ne_0 : G.map x₀ ≠ 0)
  : RightDeriv (F / G) x₀ ((A * G.map x₀ - F.map x₀ * B) / (G.map x₀)^2)
:= sorry
...
:= by
  ...
  | some A =>
    ...
    | some B =>
      ...
      have h_fg_eq : D₊ (f / g) x₀ = the ((A * g x₀ - f x₀ * B) / (g x₀)^2) := by
        apply RightDeriv_to_RightDerivExpr
        convert RightDeriv.Div h_f h_g h_g_ne_0 using 2
        simp only [HDiv.hDiv, Div.div, Function_op₂]
        rw [Set.inter_self]
      rw [h_fg_eq]
      have h_denom_neq : the ((g x₀)^2) ≠ the 0 := by
