...
/-- ### 数列收敛
    ### Sequence Converges -/
abbrev SeqConverges (a : Sequence) : Prop :=
  ∃ L : ℝ, SeqLimit a L
...
def FuncLimit (F : Function) (x₀ L : ℝ) : Prop :=
  ∀ ε > 0, ∃ δ > 0, ∀ x, |x - x₀| ∈ Ioo 0 δ → |F.map x - L| < ε

/-- ### 函数收敛
    ### Function Converges -/
abbrev FuncConverges (F : Function) (x₀ : ℝ) : Prop :=
  ∃ L : ℝ, FuncLimit F x₀ L
...
def LeftLimit (F : Function) (x₀ L : ℝ) : Prop :=
  ∀ ε > 0, ∃ δ > 0, ∀ x ∈ Ioo (x₀ - δ) x₀, |F.map x - L| < ε

/-- ### 左收敛
    ### Left Converges -/
abbrev LeftConverges (F : Function) (x₀ : ℝ) : Prop :=
  ∃ L : ℝ, LeftLimit F x₀ L
...
def RightLimit (F : Function) (x₀ L : ℝ) : Prop :=
  ∀ ε > 0, ∃ δ > 0, ∀ x ∈ Ioo x₀ (x₀ + δ), |F.map x - L| < ε

/-- ### 右收敛
    ### Right Converges -/
abbrev RightConverges (F : Function) (x₀ : ℝ) : Prop :=
