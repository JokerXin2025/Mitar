...
def Rule_And_Word : Rule := fun expr expr_rec styles fvars => do
  if expr.isAppOfArity ``And 2 then
    ...
    let A ← expr_rec args[0]! .Text .only styles fvars
    let B ← expr_rec args[1]! .Text .only styles fvars
...
def Rule_Not_Word : Rule := fun expr expr_rec styles fvars => do
  if expr.isAppOfArity ``Not 1 then
    ...
    let A ← expr_rec arg .Text .only styles fvars
...
def Rule_Implies_Symbol : Rule := fun expr expr_rec styles fvars => do
  ...
  | .forallE _ binder body _ => do
      if expr.isArrow then
        let XXX ← expr_rec binder .Implies .left styles fvars
        let YYY ← expr_rec body .Implies .right styles fvars
  ...
  if expr.isForall then
    if !expr.isArrow then
      let res ← forallTelescope expr fun _fvars body => do
        let XXX ← expr_rec body .Text .only styles fvars
        match _fvars.size with
        | 1 =>
          let x ← expr_rec _fvars[0]! .Rel .left styles fvars
          let A ← expr_rec (← inferType _fvars[0]!) .Rel .right styles fvars
        ...
        | 2 =>
          ...
          | true, true =>
            let A ← expr_rec typeExpr1 .Text .only styles fvars
            let B ← expr_rec typeExpr2 .Text .only styles fvars
            return s!"若{A}且{B}, 则{XXX}"
          | true, false => return ""
          | false, true =>
            let x ← expr_rec _fvars[0]! .Rel .left styles fvars
            let A ← expr_rec typeExpr1 .Rel .right styles fvars
            let B ← expr_rec typeExpr2 .Text .only styles fvars
            return s!"对任意满足{B}的 ${x}\\in {A}$ 有{XXX}"
          | false, false =>
            let x ← expr_rec _fvars[0]! .Rel .left styles fvars
            let y ← expr_rec _fvars[1]! .Rel .left styles fvars
            let A ← expr_rec typeExpr1 .Rel .right styles fvars
            let B ← expr_rec typeExpr2 .Rel .right styles fvars
            return s!"{XXX}对一切 ${x}\\in {A}$ 和 ${y}\\in {B}$ 成立"
        | _ =>
          let mut xAyB_array := #[]
          for fvar in _fvars do
            let x ← expr_rec fvar .Rel .left styles fvars
            let A ← expr_rec (← inferType fvar) .Rel .right styles fvars
...
def Rule_Exists_Word : Rule := fun expr expr_rec styles fvars => do
  if expr.isAppOfArity ``Exists 2 then
    let res ← existsTelescope expr fun _fvars safeBody => do
      ...
      let XXX ← expr_rec safeBody .Text .only styles fvars
